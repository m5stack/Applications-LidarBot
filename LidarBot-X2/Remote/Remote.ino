/**
   ...
   the SSID of slave for an easy setup) Step 3 : Once found, add Slave as peer
...
void setup() {
    ...
    WiFi.onEvent(WiFiGotIP, WiFiEvent_t::ARDUINO_EVENT_WIFI_AP_STACONNECTED);
...
void readAddressFromRom() {
    ...
    if (mac_addr.equals("error")) {
        ...
        sscanf(mac_addr.c_str(), "%x:%x:%x:%x:%x:%x", &slave_set.peer_addr[0],
               &slave_set.peer_addr[1], &slave_set.peer_addr[2],
               &slave_set.peer_addr[3], &slave_set.peer_addr[4],
...
void loop() {
    ...
    // keyBoard.keyData[1], keyBoard.keyData[2], keyBoard.keyData[3],
    // keyBoard.keyData[4]);
